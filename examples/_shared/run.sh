#!/usr/bin/env bash
# Avvia Ollama, rizzo-pii e Annona per un esempio, poi apre l'interfaccia.
# Restano accesi in background; ./stop.sh li spegne. Un esempio alla volta:
# tutti usano la porta 7070.
set -euo pipefail
...
if ! up localhost:11434/api/tags; then (exec nohup ollama serve >"$TOOLS/ollama.log" 2>&1) & fi
if ! up localhost:5005/health; then (cd "$TOOLS/rizzo-pii" && exec nohup env/bin/python src/app/app.py >"$TOOLS/rizzo.log" 2>&1) & fi
...
echo
echo "Esempio:     $(basename "$KIT")"
echo "Pratiche:    $KIT/Pratiche"
